...
class Cotizacion {
    ...
    public function __construct($id = null, $nombre_proyecto = null, $cliente = null, $total = null, $fecha = null) {
        $this->id = $id;
    ...
    public static function getAll() {
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $cliente = Cliente::getById($row['cliente_id_cliente']);
    ...
    public function save() {
        try {
            ...
            if($this->id != null) {
                ...
                $stmt = $db->prepare("UPDATE cotizacion SET proyecto = ?, cliente_id_cliente = ?, total = ? WHERE cotizacion_id = ?");
                $stmt->execute([$this->nombre_proyecto, $this->cliente->getId(), $this->total, $this->id]);
            } else {
                // Insert new record
                $stmt = $db->prepare("INSERT INTO cotizacion (proyecto, cliente_id_cliente, total, fecha) VALUES (?, ?, ?, NOW())");
                $stmt->execute([$this->nombre_proyecto, $this->cliente->getId(), $this->total]);
